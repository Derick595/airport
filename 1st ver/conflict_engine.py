...
def suggest_fix(
    ...
    if conflict_type == "gate":
        ...
        for g in gates:
            ...
            if is_gate_available(g["id"], ds_start, ds_end, flights, conflicting_flight_id, buffer_mins):
                return {
                    ...
                    "no_solution": False,
                    "reason": (
                        f"AI suggested that since there is a time conflict on {conflict.get('resource_name', current_gate_id)}, "
                        f"it is recommended to shift Flight {downstream_flight.get('flight_no', conflicting_flight_id)} "
        ...
        return {
            ...
            "no_solution": True,
            "reason": (
                f"AI Analysis: Severe time conflict on {conflict.get('resource_name', current_gate_id)}. "
        ...
        for c in crew:
            ...
            if is_crew_available(c["id"], ds_start, ds_end, flights, conflicting_flight_id, buffer_mins):
                return {
                    ...
                    "no_solution": False,
                    "reason": (
                        f"AI suggested that since there is a duty time conflict on {conflict.get('resource_name', current_crew_id)}, "
                        f"it is recommended to shift Flight {downstream_flight.get('flight_no', conflicting_flight_id)} "
        ...
        return {
            ...
            "no_solution": True,
            "reason": (
                f"AI Analysis: Duty overlap on {conflict.get('resource_name', current_crew_id)}, but all {len(crew)} "
