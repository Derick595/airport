        ...
        <div class="ai-recommendation-box ${isNoSolution ? 'no-solution' : ''}">
          <div class="rec-content-block">
            <span class="rec-label">
              ${isNoSolution ? '🚨 DISPATCH ALERT: MANUAL ACTION REQUIRED' : '💡 AI COPILOT RECOMMENDATION'}
            </span>
...
async function reassignResource(flightId, resourceType, newResourceId) {
  try {
    ...
    addAuditLog(
      `AI Suggested Shift Executed: Flight ${data.flight.flight_no} reassigned to ${newResourceId}.`,
      "RESOLVED",
      "badge-green"
    );
