...
const DispatchModule = {
  ...
  formatElapsed(startAt, endAt, isActive) {
    const start = Date.parse(startAt);
    const end = endAt ? Date.parse(endAt) : (isActive ? Date.now() : NaN);
  ...
  updateTimers() {
    document.querySelectorAll("[data-run-timer]").forEach(el => {
      el.textContent = this.formatElapsed(el.dataset.start, el.dataset.end, el.dataset.active === "true");
    });
  },
  ...
  renderRecovery(plan) {
    ...
    if (!plan) return;
    if (!plan.affected_runs.length) {
      container.textContent = "No queued or blocked runs use this lot.";
      return;
    }
    plan.affected_runs.forEach(run => {
      const card = document.createElement("div");
      card.className = "bg-slate-900/70 border border-slate-700 rounded-lg p-3 text-xs text-slate-200";
      const option = run.options[0];
      const summary = document.createElement("div");
      summary.textContent = `${run.order_number} (${run.run_id}) · ${run.status} · ${run.remaining_units} units remaining · ${run.material_needed} material needed`;
      card.appendChild(summary);
      const detail = document.createElement("div");
      detail.className = "text-slate-400 mt-1";
      detail.textContent = option ? `Suggested: ${option.material_lot_id}, ${option.machine_id}, ${option.operator_id}` : run.reason;
      card.appendChild(detail);
      if (option) {
        const button = document.createElement("button");
        button.className = "mt-2 px-3 py-1.5 rounded bg-amber-600 hover:bg-amber-500 text-white font-semibold";
        button.textContent = "Review and dispatch";
        button.addEventListener("click", () => this.openRecoveryOption(run, option));
  ...
  openRecoveryOption(run, option) {
    this.openModal(run.run_id, run.order_number, option.material_lot_id);
    document.getElementById("dispatch-machine-select").value = option.machine_id;
    document.getElementById("dispatch-operator-select").value = option.operator_id;
    document.getElementById("dispatch-material-select").value = option.material_lot_id;
  },
  ...
  renderRuns(runs) {
    ...
    runs.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td class="py-3 px-3 text-slate-300 font-mono text-2xs" data-run-timer data-start="${r.start_time || ''}" data-end="${r.end_time || ''}" data-active="${['RUNNING', 'BLOCKED'].includes(r.status)}">—</td>
