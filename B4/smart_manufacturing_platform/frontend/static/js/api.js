...
const API = {
  ...
  async getMaterialRecovery(lotId) {
    const res = await fetch(`/api/recovery/material/${encodeURIComponent(lotId)}`);
    if (!res.ok) throw new Error(`Failed to plan recovery for ${lotId}`);
    return await res.json();
  },
  ...
  async toggleSimulator(running, speed) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || "Failed to toggle simulator");
    }
    return await res.json();
  },
