...
const App = {
  ...
  async init() {
    ...
    WSManager.on("SCENARIO_TRIGGERED", () => this.refresh());
    ...
    if (window.WhatIfLab) WhatIfLab.init();

    // 5. Setup Top-Bar Controls
    this.setupControls();
    DispatchModule.startTimer();
  },
  ...
  async refresh() {
    try {
      ...
      const heldLot = this.data.materials.find(m => m.lot_id === "LOT-RESIN-204" && m.status === "ON_HOLD");
      if (window.DispatchModule) {
        const recovery = heldLot ? await API.getMaterialRecovery(heldLot.lot_id) : null;
        DispatchModule.renderRecovery(recovery);
      }
  ...
  renderTopBar(data) {
    ...
    if (promptBanner) {
      promptBanner.classList.toggle("hidden", !onHoldLot);
      const details = document.getElementById("prompt-alert-details");
      if (details && onHoldLot) {
        const affected = data.production_runs.filter(r => r.required_material_lot_id === onHoldLot.lot_id && ["SCHEDULED", "BLOCKED"].includes(r.status));
        details.textContent = `${onHoldLot.lot_id} is on hold. ${affected.length} queued or blocked run(s) need review. ${onHoldLot.hold_reason || "Awaiting quality decision."}`;
      }
    }
  },
  ...
  setupControls() {
    document.getElementById("btn-toggle-sim").addEventListener("click", async () => {
      const isRunning = this.data ? this.data.simulation_running : true;
      try {
        const result = await API.toggleSimulator(!isRunning);
        if (this.data) {
          this.data.simulation_running = result.running;
          this.renderTopBar(this.data);
        }
        await this.refresh();
      } catch (err) {
        alert("Simulator control failed: " + err.message);
      }
    });

    ["1x", "2x", "5x"].forEach(speedStr => {
      const speedVal = parseFloat(speedStr);
      document.getElementById(`btn-speed-${speedStr}`).addEventListener("click", async () => {
        try {
          await API.toggleSimulator(undefined, speedVal);
          ["1x", "2x", "5x"].forEach(s => {
          ...
          alert("Simulator speed change failed: " + err.message);
        }
      });
    });
