            ...
            try:
                ...
                if run_row["status"] != "SCHEDULED":
                    raise ResourceConflictException(
                        f"Production run '{run_id}' is {run_row['status']} and cannot be dispatched.",
                        "RUN_NOT_DISPATCHABLE", run_id
                    )
                if required_qty <= 0:
                    raise ResourceConflictException("Required material quantity must be positive.", "INVALID_QUANTITY", material_lot_id)
                ...
                if op_row["certification_expires"] and op_row["certification_expires"] < datetime.now().date().isoformat():
                    raise ResourceConflictException(
                        f"Operator '{operator_id}' certification has expired.",
                        "OPERATOR_CERTIFICATION_EXPIRED", operator_id
                    )
                ...
                if mat_row["status"] == "EXPIRED" or mat_row["expiration_date"] < datetime.now().date().isoformat():
                    raise ResourceConflictException(
                ...
                if mat_row["status"] != "AVAILABLE" or mat_row["reserved_by_run_id"]:
                    raise ResourceConflictException(
                        f"Material Lot '{material_lot_id}' is {mat_row['status']} and cannot be reserved.",
                        "MATERIAL_UNAVAILABLE", material_lot_id
                    )
    ...
    def release_locks(run_id: str, new_status: str = "COMPLETED") -> Dict[str, Any]:
        ...
        with _LOCK:
            ...
            try:
                ...
                cursor.execute("SELECT 1 FROM resource_locks WHERE run_id = ? AND lock_status = 'HELD' LIMIT 1", (run_id,))
                if not cursor.fetchone():
                    raise ValueError(f"Run '{run_id}' has no active allocation to release.")
                ...
                if machine_id:
                    cursor.execute("""
                    UPDATE machines 
                    SET status = CASE WHEN status = 'MAINTENANCE' THEN 'MAINTENANCE' ELSE 'IDLE' END,
                        current_run_id = NULL
                ...
                if lot_id:
                    # Check remaining qty
                    cursor.execute("SELECT remaining_qty, status FROM materials WHERE lot_id = ?", (lot_id,))
                    mat_row = cursor.fetchone()
                    new_mat_status = mat_row["status"] if mat_row and mat_row["status"] in ("ON_HOLD", "QUARANTINED", "EXPIRED") else ('DEPLETED' if mat_row and mat_row["remaining_qty"] <= 0 else 'AVAILABLE')
                    cursor.execute("""
                    UPDATE materials 
    ...
    def release_material_hold(lot_id: str) -> Dict[str, Any]:
        ...
        with _LOCK:
            ...
            try:
                ...
                cursor.execute("SELECT lot_id, status, reserved_by_run_id, remaining_qty, expiration_date FROM materials WHERE lot_id = ?", (lot_id,))
                mat = cursor.fetchone()
                if not mat:
                    raise ValueError(f"Material lot '{lot_id}' not found.")
                if mat["status"] != "ON_HOLD":
                    raise ValueError(f"Material lot '{lot_id}' is not on hold.")

                new_status = "IN_USE" if mat["reserved_by_run_id"] else ("EXPIRED" if mat["expiration_date"] < datetime.now().date().isoformat() else ("DEPLETED" if mat["remaining_qty"] <= 0 else "AVAILABLE"))
                ...
                cursor.execute("COMMIT;")
                return {"success": True, "lot_id": lot_id, "status": new_status}
            except Exception as e:
                cursor.execute("ROLLBACK;")
                raise e
            finally:
