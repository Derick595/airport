...
from backend.models import (
    ...
    SimulatorControlRequest, WhatIfRequest
)
...
from backend.recovery import plan_material_recovery as build_material_recovery_plan
...
@router.get("/recovery/material/{lot_id}")
def plan_material_recovery(lot_id: str):
    try:
        return build_material_recovery_plan(lot_id)
...
async def create_production_run(run: ProductionRunCreate):
    ...
    try:
        ...
        run_id = f"RUN-{datetime.now().year}-{uuid4().hex[:8].upper()}"
...
async def dispatch_run(req: ProductionRunDispatch):
    ...
    try:
        conn = get_connection()
        try:
            row = conn.execute("SELECT required_material_qty FROM production_runs WHERE id = ?", (req.run_id,)).fetchone()
        finally:
            conn.close()
        if row is None:
            raise ResourceConflictException(f"Production run '{req.run_id}' not found.", "RUN_NOT_FOUND", req.run_id)
        res = LockManager.allocate_and_lock(
            run_id=req.run_id,
            machine_id=req.machine_id,
            operator_id=req.operator_id,
            material_lot_id=req.material_lot_id,
            required_qty=row["required_material_qty"]
        )
...
def run_what_if(req: WhatIfRequest):
    try:
        return analyze_what_if(req.scenario_type, req.resource_id)
    except ValueError as error:
...
async def toggle_simulator(req: SimulatorControlRequest):
    if req.speed is not None:
        simulator.simulation_speed = max(0.5, min(5.0, req.speed))
...
async def trigger_scenario(req: ScenarioTriggerRequest):
    ...
    elif req.scenario_name == "machine_fault":
        res = simulator.trigger_machine_fault_scenario()
    elif req.scenario_name == "material_shortage":
        res = simulator.trigger_material_shortage_scenario()
