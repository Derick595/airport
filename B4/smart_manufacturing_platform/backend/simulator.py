...
class FactorySimulator:
        ...
        try:
            ...
            for run in active_runs:
                ...
                if run["material_status"] == "ON_HOLD":
                    ...
                    """, (run_id,))
                    continue
                if run["material_status"] not in ("IN_USE", "AVAILABLE") or run["remaining_qty"] is None or run["remaining_qty"] < run["required_material_qty"]:
                    cursor.execute("UPDATE machines SET status = 'BLOCKED' WHERE id = ?", (mach_id,))
                    cursor.execute("UPDATE production_runs SET status = 'BLOCKED', blocked_reason = 'Material unavailable or insufficient for next unit' WHERE id = ?", (run_id,))
                    continue
                ...
                if random.random() < 0.85:
                    ...
                    # Deduct raw material
                    new_mat_qty = run["remaining_qty"] - run["required_material_qty"]
    ...
    def trigger_machine_fault_scenario(self) -> Dict[str, Any]:
        """Take CNC-01 offline for maintenance and block its active run."""
        conn = get_connection()
        try:
            conn.execute("BEGIN IMMEDIATE")
            machine = conn.execute("SELECT status, current_run_id FROM machines WHERE id = 'CNC-01'").fetchone()
            if not machine:
                raise ValueError("Demo machine CNC-01 is missing")
            run_id = machine["current_run_id"]
            conn.execute("UPDATE machines SET status = 'MAINTENANCE' WHERE id = 'CNC-01'")
            if run_id:
                conn.execute("UPDATE production_runs SET status = 'BLOCKED', blocked_reason = 'CNC-01 spindle drive fault; maintenance required' WHERE id = ? AND status = 'RUNNING'", (run_id,))
            alert_id = f"ALT-FAULT-{uuid4().hex[:12]}"
            conn.execute("""
                INSERT INTO alerts (id, timestamp, severity, source_type, source_id, title, message, recommended_action, status)
                VALUES (?, ?, 'CRITICAL', 'MACHINE', 'CNC-01', ?, ?, ?, 'ACTIVE')
            """, (alert_id, datetime.now().isoformat(), "CNC-01 spindle drive fault", f"CNC-01 requires maintenance. Active run {run_id or 'none'} must not continue until the fault is cleared.", "Inspect the spindle drive, then review the affected run and its resource locks."))
            conn.commit()
            return {"scenario": "Machine fault", "machine_id": "CNC-01", "affected_run_id": run_id, "alert_id": alert_id}
        except Exception:
            conn.rollback()
            raise
        finally:
    ...
    def trigger_material_shortage_scenario(self) -> Dict[str, Any]:
        """Deplete the SMT substrate lot and stop its run before another unit is made."""
        conn = get_connection()
        try:
            conn.execute("BEGIN IMMEDIATE")
            lot = conn.execute("SELECT remaining_qty, reserved_by_run_id FROM materials WHERE lot_id = 'LOT-PCB-305'").fetchone()
            if not lot:
                raise ValueError("Demo material LOT-PCB-305 is missing")
            run_id = lot["reserved_by_run_id"]
            conn.execute("UPDATE materials SET remaining_qty = 0, status = 'DEPLETED' WHERE lot_id = 'LOT-PCB-305'")
            if run_id:
                conn.execute("UPDATE production_runs SET status = 'BLOCKED', blocked_reason = 'LOT-PCB-305 depleted; no material for next unit' WHERE id = ? AND status = 'RUNNING'", (run_id,))
                conn.execute("UPDATE machines SET status = 'BLOCKED' WHERE current_run_id = ?", (run_id,))
            alert_id = f"ALT-STOCK-{uuid4().hex[:12]}"
            conn.execute("""
                INSERT INTO alerts (id, timestamp, severity, source_type, source_id, title, message, recommended_action, status)
                VALUES (?, ?, 'CRITICAL', 'MATERIAL', 'LOT-PCB-305', ?, ?, ?, 'ACTIVE')
            """, (alert_id, datetime.now().isoformat(), "PCB substrate lot depleted", f"LOT-PCB-305 has zero remaining stock. Active run {run_id or 'none'} is stopped before further consumption.", "Replenish with a validated FR4-ML8 lot and review the blocked order."))
            conn.commit()
            return {"scenario": "Material shortage", "lot_id": "LOT-PCB-305", "affected_run_id": run_id, "alert_id": alert_id}
        except Exception:
            conn.rollback()
            raise
        finally:
