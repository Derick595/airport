...
6. **What-If Decision Lab**: A read-only digital-twin sandbox that previews the order, unit, and delay impact of a machine failure, material hold, or operator absence and proposes available recovery options before a supervisor commits a change.
...
### F. What-If Decision Lab (`WhatIfEngine`)
- Simulates machine failure, material quality hold, and operator absence against the current factory state.
- Scores operational risk from affected runs, remaining units, overdue orders, and available alternatives, with the formula shown in the result.
- Recommends qualified machines, compatible material batches, or certified operators where available.
- Is strictly read-only: simulations never reserve, release, consume, or modify production resources.
...
The platform includes 5 interactive incident scenarios plus a reset control in the **Interactive Scenarios** tab. Reset to the baseline before each scenario for a repeatable demo.
...
3. **Scenario 3: Defect Genealogy & Patient Zero Audit**
   ...
   - Runs root-cause analysis and identifies exposed parts for quarantine. The displayed confidence is illustrative, not a statistically calibrated probability.

4. **Scenario 4: CNC Spindle Drive Fault**
   - Puts `CNC-01` into maintenance and blocks its active run.
   - Preserves resource locks for incident review; aborting the run releases its locks but does not make the faulty machine available.

5. **Scenario 5: PCB Substrate Shortage**
   - Depletes `LOT-PCB-305`, blocks the active SMT run, and prevents another unit from consuming missing stock.
   - Raises a critical inventory alert.
